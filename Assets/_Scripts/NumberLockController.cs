...
public class NumberLockController : MonoBehaviour
{
    ...
    public List<TMPro.TextMeshProUGUI> lock_numbers = new List<TMPro.TextMeshProUGUI>();
    public List<int> lock_answer = new List<int>();
    ...
    private bool CheckLock()
    {
        ...
        for(int i = 0; i < lock_numbers.Count; i++)
        {
            if (int.Parse(lock_numbers[i].text) != lock_answer[i]) checkLock = false;
        }
